...
//Jumping
//Note: allowing jumping while pressing down for dash characters so that you can aim down and jump simultaneously
if (canMove || (isThrow and room != rmWeaponGet) || onRushJet) && ground && global.keyJumpPressed && (!global.keyDown || ((global.character.dashCharacter) && (global.DashCharSlideButtonOnly)))
{
...
//Sliding
if enableSlide {
    if (global.dashAssist){
        //Assisted Semi-Auto Slide (giving margin for pressing slide key early/late)
        //dashInputMargin = How sloppy can we be with dash timing?
        var dashInputMargin = cfgKeySlideMaxHoldTime/2;
        if (global.character.dashCharacter && !(global.character.isThrow || global.character.isShoot)) {
            if (slideTimer >= slideFrames-dashInputMargin || slideTimer < dashInputMargin){
                if (global.enableSlideKey){
                        global.keySlidePressed = global.keySlide;
                }
                if(!global.DashCharSlideButtonOnly && keyDown){
                    global.keyJumpPressed = global.keyJump;
                }
            }
        }
    }

    //only attempt combo slide if we are not a dash character or we decide to allow it in options
    if ground && 
        //We are a dash character
        ((global.character.dashCharacter &&
            //and we hit the slide button
            ((global.enableSlideKey && global.keySlidePressed)
            //or are allowed to combo input slide
            ||(!global.DashCharSlideButtonOnly && global.keyJumpPressed && global.keyDown)))
        //or we are a non-dash character
        ||(!global.character.dashCharacter && !isSlide && ((global.keyJumpPressed && global.keyDown)||(global.enableSlideKey && global.keySlidePressed))))
...
with (objMass){
    if (instance_exists(prtPlayer)){
        var delta_x, delta_y, theta, accel;
        delta_x = x-prtPlayer.x;
        delta_y = prtPlayer.y-y;
        theta = arctan(delta_y/delta_x);
        accel = 20/sqrt(sqr(delta_x)+sqr(delta_y));
        prtPlayer.accel_x += sign(delta_x)*floor(lengthdir_x(accel,radtodeg(theta)));
        prtPlayer.accel_y += sign(delta_y)*floor(lengthdir_y(accel,radtodeg(theta)));
    }
}
accel_x = clamp(accel_x,-2*maxVspeed,2*maxVspeed);
accel_y = clamp(accel_y,-maxVspeed,maxVspeed);
global.xspeed += accel_x;
global.yspeed += accel_y;
global.xspeed = clamp(global.xspeed,-2*maxVspeed,2*maxVspeed);
global.yspeed = clamp(global.yspeed,-maxVspeed,maxVspeed);
*/
