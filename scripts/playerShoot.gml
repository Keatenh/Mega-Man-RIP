...
//&&(!enableSlide || (!global.dashAssist || (!global.character.dashCharacter || 
//!(slideTimer >= slideFrames-dashInputMargin || slideTimer < dashInputMargin && slideTimer > 0))))
//^^ Last 2 lines to try to stop bug of being able to shoot during grace period for continuous sliding with Dash Assist
//Currently removed and instead disabled dash assist during isThrow || isShoot
{   
    if climbing {
...
if isShoot {
    ...
    //updating sprite properly without moving
